...
setwd(resultsDir)
fit <-foreach(i_im=1:length(image_names),.combine='rbind') %do%
{ 
...
g <- ggplot(ff, aes(x=year,y=par1,colour=model_set,shape=model_name)) +  theme_bw() + geom_point(position=pd)+ 
	geom_errorbar(aes(ymin=par1-se, ymax=par1+se),width=0.0,position=pd) + ylab(expression(alpha))
...
g + geom_line(aes(group=1),colour="lightgreen") + ylab(expression(X[min]))
ggsave("EstimatedXmin_year.png",plot=g,width=6,height=4,units="in",dpi=600)


# Exponential RATES ARE AN ARTIFACT!!!!!!!!!!!!!!!!!!!!!!! 
# MAKE SIMULATIONS TO TEST THE PROCEDURE
#
ff <- filter(all_fit,delta_AICc==0,model_set=="<Xmin") %>% mutate(year=substr(data_set_name,16,19),se=(par1-1)/sqrt(n))

g <- ggplot(ff, aes(x=year,y=par2)) +  theme_bw() + geom_point(shape=19,colour="red")
g + geom_line(aes(group=1),colour="lightgreen") + ylab("Exponential rate")
g
setwd(oldcd)
