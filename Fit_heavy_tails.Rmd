...
ff <- filter(all_fit,model_name=="Power", model_set!="Xmin=9") %>% 
	mutate(year=substr(data_set_name,47,50), se=(par1-1)/sqrt(n))
...
g <- ggplot(ff, aes(x=year,y=par1,colour=model_set,shape=model_name)) +  theme_bw() + geom_point(position=pd)+ 
	...
	geom_errorbar(aes(ymin=lPar1, ymax=hPar1),width=0.5,position=pd) + ylab(expression(alpha)) + facet_wrap(~subregion)
g
