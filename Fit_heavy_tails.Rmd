...
m0.gls <-gls(par1 ~ region+year,data=ff,weights = http://www.fromthebottomoftheheap.net/2011/06/10/what-is-ordisurf-doing/http://www.fromthebottomoftheheap.net/2011/06/10/what-is-ordisurf-doing/varFixed(~weight), correlation = corAR1(form=~year| region),method="ML")
anova(m1.gls,m0.gls)
...
g <- ggplot(ff1, aes(x=year,y=par1,colour=region),shape=21) +  theme_bw() + geom_point(position=pd,size=1.5)+ 
	geom_errorbar(aes(ymin=lPar1, ymax=hPar1),width=0.5,position=pd) + ylab(expression(alpha)) +
	scale_colour_brewer(palette="Set1",name="Region",labels=c("AF", "EUAS", "NA","SAST","SEAS")) + theme(legend.position="bottom") +  theme(axis.text.x = element_text(angle = 90, hjust = 0)) + scale_x_continuous(breaks=2000:2014)
PowerExp_Xmin_year
...
PowerExp_Xmin_yearPowerExp_Xmin_year
...
g + ylab("Largest patch proportion")  +scale_colour_brewer(palette="Set1",name="Region") +
...
g + ylab("Largest patch proportion")  +scale_colour_brewer(palette="Set1",name="Region",guide=F) +
...
g + ylab(expression(Delta~year~Largest~Patch~km^2))  +scale_colour_brewer(palette="Set1",name="Region",guide=FALSE) + stat_quantile() +
...
#
# Annual Fluctuations in proportion of total patch area
#

g <- ggplot(ff1, aes(y=abs(delta_year_prop_patch),x=year,colour=region)) +  theme_bw() +
	geom_point(shape=19) + facet_wrap(~region) #geom_smooth(method = "ml",se=F)
g + ylab(expression(Delta~year~Largest~Patch/Total~area))  +scale_colour_brewer(palette="Set1",name="Region",guide=F) + stat_quantile() +
	theme(axis.text.x = element_text(angle=90,hjust=0)) #+ geom_crossbar(aes(ymax = abs(delta_year_patch), ymin = 0)) 
ggsave("figure/Annual_Delta_prop_patch_year_gt1e07.png",width=8,height=8,units="in",dpi=600)
...
#
# Fluctuation around the mean propotional to total patch area
#
g <- ggplot(ff1, aes(y=(delta_prop_max_patch),x=year,colour=region)) +  theme_bw() +
	geom_point(shape=19) + facet_wrap(~region) #geom_smooth(method = "ml",se=F)
g + ylab("Largest Patch Fluctuations/Total patch area")  +scale_colour_brewer(palette="Set1",name="Region",guide=FALSE) + 
	stat_quantile(quantiles=c(.10,.50,.90)) +
	theme(axis.text.x = element_text(angle=90,hjust=0)) #
...
qu <-group_by(ff1,region,subregion) %>% do(data.frame(t(coef(summary(rq((delta_prop_max_patch)~year,data=.,tau=c(.10)),se="boot",R=999))[2,]))) %>% mutate(tau=.10, group="gt1e07")

qu <- bind_rows(qu,group_by(ff1,region,subregion) %>% do(data.frame(t(coef(summary(rq((delta_prop_max_patch)~year,data=.,tau=c(.50)),se="boot",R=999))[2,]))) %>% mutate(tau=.50, group="gt1e07"))

qu <- bind_rows(qu,group_by(ff1,region,subregion) %>% do(data.frame(t(coef(summary(rq((delta_prop_max_patch)~year,data=.,tau=c(.90)),se="boot",R=999))[2,]))) %>% mutate(tau=.90, group="gt1e07"))
...
#
# Annual delta patch proportion
#

g <- ggplot(ff1, aes(y=abs(delta_year_prop_patch),x=year,colour=region)) +  theme_bw() +
	geom_point(shape=19) + facet_wrap(~region1)
g + ylab("Delta year Maximun patch")  +scale_colour_brewer(palette="Set1",name="Sub-Region",guide=F) +
	theme(axis.text.x = element_text(angle=90,hjust=0)) + stat_quantile()
ggsave("figure/Annual_Delta_prop_patch_year_ls1e07.png",width=8,height=8,units="in",dpi=600)
...
g <- ggplot(ff1, aes(y=delta_prop_max_patch,x=year,colour=region)) +  theme_bw() +
	geom_point(shape=19) + facet_wrap(~region1)
g + ylab("Largest Patch Fluctuations/Total patch area") + scale_colour_brewer(palette="Set1",name="Sub-Region",guide=F) +
	theme(axis.text.x = element_text(angle=90,hjust=0)) +
	stat_quantile(quantiles=c(.10,.50,.90))
	# geom_crossbar(aes(ymax = (delta_max_patch)/total_patch_area, ymin = 0)) 
...
qu <- bind_rows(qu,group_by(ff1,region,subregion) %>% do(data.frame(t(coef(summary(rq((delta_prop_max_patch)~year,data=.,tau=c(.10)),se="boot",R=999))[2,]))) %>% mutate(tau=.10, group="ls1e07"))

qu <- bind_rows(qu, group_by(ff1,region,subregion) %>% do(data.frame(t(coef(summary(rq((delta_prop_max_patch)~year,data=.,tau=c(.50)),se="boot",R=999))[2,]))) %>% mutate(tau=.50, group="ls1e07"))

qu <- bind_rows(qu,group_by(ff1,region,subregion) %>% do(data.frame(t(coef(summary(rq((delta_prop_max_patch)~year,data=.,tau=c(.90)),se="boot",R=999))[2,]))) %>% mutate(tau=.90, group="ls1e07"))
names(qu)[4]<-"StdError"
names(qu)[5]<-"t_value"
names(qu)[6]<-"p_value"

qu <- arrange(qu,region,subregion) %>% 
	mutate(Value=round(Value,6),StdError=round(StdError,6),t_value=round(t_value,6),p_value=round(p_value,5))
...
ku <-do(ff1,dmp_skewness=descdist(.$delta_max_patch, discrete=FALSE, boot=500)$skewness)
ku <-do(ff1,dmpp_skewness=descdist(.$delta_max_patch/.$total_patch_area, discrete=FALSE, boot=500)$skewness)
...
ff1<-pstat %>% group_by(region,subregion) %>% mutate(period=rep(1:2,each=8,length.out=15))
...
#
dev.off()
do(ff1,data.frame(matrix(acf(.$delta_year_prop_patch,plot=T)$acf,nrow=1)))
...
#
ff2 <-do(ff1, data.frame(lnested.test(.$delta_year_prop_patch,.$period,bootstrap = T,num.bootstrap = 999,correction.method="zero.correction")$N[2]))
...
# 14     OC         7 0.64864865
# 15     OC         8 0.08608609
# 16   SAST         1 0.58658659
# 17   SAST         2 0.10710711
# 18    SAT         1 0.99299299
# 19   SEAS         1 0.04004004
# 20   SEAS         2 0.09109109

names(ff2)[3] <- "Increase"
#pandoc.table(ff2[,c(1,2,4,3)])

ff3 <-do(ff1, data.frame(lnested.test(.$delta_year_prop_patch,.$period,tail="left",bootstrap = T,num.bootstrap = 999,correction.method="zero.correction")$N[2]))
...
# Fluctuation in absolute max patch area 
#
ff1<-pstat %>% group_by(region,subregion) %>% mutate(period=rep(1:3,each=5,length.out=15),delta_prop_max_patch=)

# Check for autocorrelations
#
do(ff1,data.frame(matrix(acf(.$delta_max_patch,plot=T)$acf,nrow=1)))

ff2 <- do(ff1, data.frame(lnested.test(.$delta_max_patch,.$period,bootstrap = T,num.bootstrap = 999,correction.method="zero.correction")$N[2]))
