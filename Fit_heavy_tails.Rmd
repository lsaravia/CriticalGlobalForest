...
g + ylab("Largest Patch Fluctuations/Total forest area")  +scale_colour_brewer(palette="Set1",name="Region",guide=FALSE) + 
...
g <- ggplot(ff1, aes(y=(delta_max_patch),x=year,colour=region)) +  theme_bw() +
	geom_point(shape=19) + facet_wrap(~region) #geom_smooth(method = "ml",se=F)
g + ylab(expression(Largest~Patch~Fluctuations~km^2))  +scale_colour_brewer(palette="Set1",name="Region",guide=FALSE) + 
	stat_quantile(quantiles=c(.10,.50,.90),linetype=2) +
	theme(axis.text.x = element_text(angle=90,hjust=0)) #
    #geom_crossbar(aes(ymax = (delta_max_patch)/total_patch_area, ymin = 0)) 
...
g <- ggplot(ff1, aes(y=delta_prop_max_patch,x=year,colour=region)) +  theme_bw() +
	geom_point(shape=19) + facet_wrap(~region1)
g + ylab("Largest Patch Fluctuations/Total forest area") + scale_colour_brewer(palette="Set1",name="Sub-Region",guide=F) +
	theme(axis.text.x = element_text(angle=90,hjust=0)) +
	stat_quantile(quantiles=c(.10,.50,.90),linetype=2)
	# geom_crossbar(aes(ymax = (delta_max_patch)/total_patch_area, ymin = 0)) 
...
g <- ggplot(ff1, aes(y=delta_max_patch,x=year,colour=region)) +  theme_bw() +
	geom_point(shape=19) + facet_wrap(~region1)
g + ylab(expression(Largest~Patch~Fluctuations~km^2)) + scale_colour_brewer(palette="Set1",name="Sub-Region",guide=F) +
	theme(axis.text.x = element_text(angle=90,hjust=0)) +
	stat_quantile(quantiles=c(.10,.50,.90),linetype=2)
	# geom_crossbar(aes(ymax = (delta_max_patch)/total_patch_area, ymin = 0)) 
