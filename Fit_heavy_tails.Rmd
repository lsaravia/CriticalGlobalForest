...
# IMPORTANT!
#
# The two classes of distribution models are fitted one with fixed Xmin=9 and another with estimated Xmin
# the models with fixed Xmin were not used in the paper, we can save a lot of cumputing time if we delete
# the fixed Xmin estimation.
#
...
```{r conheavytailfitEUAS1_merged, eval=F,echo=T,message=T,warning=T}
...
# remove from all_fit
#
euas_merged2patch <- all_fit  %>% filter(subregion=="1M")

all_fit <- all_fit  %>% filter(subregion!="1M")
