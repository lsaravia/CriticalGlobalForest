...
```{r conheavytailfitSA, eval=T,echo=T,message=T,warning=T}
#require(doParallel)
require(plyr)
require(dplyr)
...
all_fit$region <- "SA" # South America
...
```{r conheavytailplotSA, eval=T,echo=F,message=T,warning=T}
require(dplyr)
require(ggplot2)
...
```{r conheavytailplotFitCA, eval=T,echo=F,message=T,warning=T}
...
g +scale_colour_brewer(palette="Set1",name="Region") 
