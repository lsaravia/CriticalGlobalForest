...
# Testing differences in alpha parameter using Mixed models

```{r alphaGLMM, eval=F,echo=T,message=T,warning=T}
...
ff <- cbind(ff,pred)
ggplot(ff,aes(year,par1)) + geom_point(shape=21) + facet_wrap(~region) + theme_bw() + geom_line(aes(y=pred))
...
ggplot(ff1,aes(year,par1,colour=region)) + geom_point(shape=21) + theme_bw() + geom_line(aes(y=pred)) + scale_colour_brewer(palette="Set1") 


ggplot(ff1,aes(region,par1)) + geom_boxplot() + geom_jitter(width = 0.2,shape=21) +  theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust=0)) + scale_colour_brewer(palette="Set1") 
