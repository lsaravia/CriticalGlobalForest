...
```{r conheavytailfitEUAS3_SAST1, eval=T,echo=T,message=T,warning=T}
...
ggsave("figure/PowerExp_gt10e7_year.png",width=6,height=8,units="in",dpi=600)
...
ggplot(tot_fit_by_model, aes(x=model_set,y=best_model_prop,fill=model_name)) + geom_bar(stat="identity", position=position_dodge()) + facet_grid(subregion~region) + theme_bw() + scale_colour_brewer(palette="Set1") + theme(axis.text.x = element_text(angle = 90, hjust = 0, vjust=0),axis.title.x = element_blank()) + ylab("Selected models proportion") +  
...
#
# Max patch proportion plot greater than 1e7
#
...
#
# Max patch/total patch area plot less than 1e7
#
ff1 <- filter(pstat,total_patch_area<1e7)
...
g <- ggplot(ff1, aes(y=delta_year_patch,x=prop_max_patch,colour=region)) +  theme_bw() +
	geom_point(shape=19) 
...
#
ff1<-filter(pstat, total_patch_area<1e7) 
g <- ggplot(ff1, aes(y=delta_year_patch,x=prop_max_patch,colour=subregion)) +  theme_bw() +
	geom_point(shape=19,size=1) + facet_wrap(~region)
...
g + ylab("Delta Year Max Patch")  +scale_colour_brewer(palette="Set1",name="Region") +
	theme(axis.text.x = element_text(angle=90,hjust=0)) + geom_crossbar(aes(ymax = abs(delta_year_patch), ymin = 0)) 
...
g <- ggplot(ff1, aes(y=abs(delta_year_patch),x=year,colour=subregion)) +  theme_bw() +
	geom_point(shape=19) + facet_wrap(~region)
g + ylab("Delta year Maximun patch")  +scale_colour_brewer(palette="Set1",name="Sub-Region") +
...
# Fit Max patch fluctuations to power law distribution
...
fit_delta<-bind_rows(fit_delta, do(ff1,fit_cont_heavy_tail_mdls(.$delta_max_patch,-1,T)))
#fit_delta1<-bind_rows(fit_delta1, do(ff1,fit_cont_heavy_tail_mdls(.$delta_max_patch,-1)))

pandoc.table(select(fit_delta, region:GOFp) %>% group_by(region,subregion) %>% arrange(AICc))
