...
# Testing python code to Fit again heavy tailed distributions EUAS 1  
...
pyfit <- pyfit %>% mutate(region=strsplit(pyfit$file_name[1],"_")[[1]][2], subregion=strsplit(pyfit$file_name[1],"_")[[1]][3],
				 threshold= strsplit(pyfit$file_name[1],"_")[[1]][4],
				 year = gsub(".*\\.A([0-9]{4}).*","\\1",file_name))
names(all_fit_threshold)


# Lognormal without restriction has mu negative which results in a 99.7% probability is outside the range of patch data
#

lNorm <- filter(pyfit, grepl("Log.*",model_name)) %>% ungroup() %>% select(model_name:xmin) %>% mutate(mu=exp(par1),si=exp(par2),low=mu/si^3,hi=mu*si^3)

lNorm %>% mutate(higher= hi>.01) %>% group_by( model_name,higher) %>% summarize(n=n()) %>% mutate(modFreq= n/sum(n))
...
#
ta <- mutate(all_fit,  xmin=round(xmin)) %>% filter(model_set=="Estimated Xmin")  # %>% filter(region %in% c('OC'))
...
# 
#  Only Estimated Xmin models and not EUAS 1M (Eurasia with merged top 2 patches)
#
...
#
# Total Proportion of best fit models
#
all_fit %>% group_by(model_set,model_name) %>% mutate(tot_by_model=n()) %>% 
	filter(delta_AICc==0) %>% summarise(best_model_prop=n()/max(tot_by_model)) %>% select(model_name,best_model_prop) %>% arrange(desc(best_model_prop)) %>% filter(model_set=="Estimated Xmin") 
