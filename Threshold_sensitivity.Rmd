...
#
ff2<- pstat_threshold %>% filter(threshold %in% c(40)) %>% group_by(regsub,threshold) %>%  summarise(mmaxpatch=mean(max_patch),mtotpatch=mean(total_patch_area)) %>% inner_join(rsmax_ci) %>% mutate_if(is.numeric,round,digits=2)
...
plot_RSmax_yearThreshold(filter(pstat_threshold,threshold %in% c(20,25,30,35,40)),"AF1")
plot_RSmax_yearThreshold(filter(pstat_threshold,threshold %in% c(20,25,30,35,40)),"EUAS1")
plot_RSmax_yearThreshold(filter(pstat_threshold,threshold %in% c(20,25,30,35,40)),"EUAS2")
plot_RSmax_yearThreshold(filter(pstat_threshold,threshold %in% c(20,25,30,35,40)),"NA1")
plot_RSmax_yearThreshold(filter(pstat_threshold,threshold %in% c(20,25,30,35,40)),"SAST1")
plot_RSmax_yearThreshold(filter(pstat_threshold,threshold %in% c(20,25,30,35,40)),"SEAS1")
plot_RSmax_yearThreshold(filter(pstat_threshold,threshold %in% c(20,25,30,35,40)),"SEAS2")
...
#
ff1<-filter(pstat_threshold, year<2015, threshold %in% c(20,25,30,35,40)) %>%  mutate(delta_max_patch=abs(delta_max_patch))
...
fft <- fit_delta_threshold %>% filter(pvalue<0.05) %>% arrange(regsub)

#
# Fit Relative max patch fluctuations to power law exponential and lognormal distributions
#

ff1<-filter(pstat_threshold, year<2015,threshold %in% c(20,25,30,35,40)) %>%  mutate(delta_max_patch=abs(delta_prop_max_patch))
fit <- call_python_powlawfit_delta_threshold(ff1,"Results",TRUE)
...
rm(ff1,ff2,fft,fit,qu1,qu2,ta,am,m.gls,mdl1,mdl2,re)
save.image()
```
