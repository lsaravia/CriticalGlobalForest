...
ggsave("figure/BestModelByThreshold25_35.png",width=8,height=6,units="in",dpi=600)
...
pstat_threshold <-filter(pstat_threshold,total_patch_area>100000,!(region=="NA"& subregion==2))
pstat_threshold <-mutate(pstat_threshold, regsub = paste0(region,subregion))
#
# Max patch plot
#
g <- ggplot(pstat_threshold, aes(y=max_patch,x=year,colour=threshold)) +  theme_bw() +
...
ta <-filter(all_fit_threshold,model_name=="Power",model_set=="Estimated Xmin") %>% mutate(regsub = paste0(region,subregion)) %>% group_by(regsub,threshold) %>% summarise(alpha=mean(par1),xmin=mean(xmin))
...
ggplot(filter(ta,mean_tot_area>1e7)%>% mutate_if(is.numeric,funs(./1e07)), aes(y=mean_tot_area,x=threshold,colour=regsub)) +  theme_bw() +
	geom_point(shape=22)+ geom_linerange(aes(ymin = low, ymax = high)) + ylab(expression(Total~patch~area~10^7~Km^2))  + scale_colour_viridis(discrete=T,name="Region",guide=FALSE) + facet_wrap(~regsub) + theme(legend.position="bottom") + theme(axis.text.x = element_text(angle=90, hjust = 0, vjust=.5)) + coord_flip()
...
require(viridis)
ggplot(rsmax_ci, aes(threshold,mRSmax,colour=regsub)) + geom_point(shape=22) + geom_linerange(aes(ymin = low, ymax = high)) +theme_bw()+ ylab(expression(paste("Average ",RS[max])))+ coord_flip() + scale_colour_viridis(discrete = T,guide=FALSE) + facet_wrap(~regsub) + theme(axis.text.x = element_text(angle=90, hjust = 0, vjust=.5)) 
ggsave("figure/RSmax_ByThreshold.png",width=6,height=6,units="in",dpi=600)

ggplot(filter(rsmax_ci,threshold %in% c(25,27.5,30,32.5,35)), aes(threshold,mRSmax,colour=regsub)) + geom_point(shape=22) + geom_linerange(aes(ymin = low, ymax = high)) +theme_bw()+ ylab(expression(paste("Average ",RS[max])))+ coord_flip() + scale_colour_viridis(discrete = T,guide=FALSE) + facet_wrap(~regsub)  + theme(axis.text.x = element_text(angle=90, hjust = 0, vjust=.5))
...
#
ff1 <- filter(pstat_threshold,regsub %in% c("AF1","EUAS1","NA1","SAST1","SEAS1"),year<2015) 
pd <-position_dodge(.3)
...
ff1<-filter(pstat_threshold, total_patch_area>1e7,year<2015)  
...
qu_treshold <- bind_rows(qu_treshold,group_by(ff1,regsub,threshold) %>% do(data.frame(t(coef(summary(rq((delta_prop_max_patch)~year,data=.,tau=c(.10)),se="boot",R=999))[2,]))) %>% mutate(tau=.10, group="Prop") %>% rename(StdError=Std..Error,t_value=t.value,p_value=Pr...t..))

qu_treshold <- bind_rows(qu_treshold, group_by(ff1,regsub,threshold) %>% do(data.frame(t(coef(summary(rq((delta_prop_max_patch)~year,data=.,tau=c(.50)),se="boot",R=999))[2,]))) %>% mutate(tau=.50, group="Prop") %>% rename(StdError=Std..Error,t_value=t.value,p_value=Pr...t..))

qu_treshold <- bind_rows(qu_treshold,group_by(ff1,regsub,threshold) %>% do(data.frame(t(coef(summary(rq((delta_prop_max_patch)~year,data=.,tau=c(.90)),se="boot",R=999))[2,]))) %>% mutate(tau=.90, group="prop") %>% rename(StdError=Std..Error,t_value=t.value,p_value=Pr...t..))

qu1 <- filter(qu_treshold, p_value<0.05) %>% arrange( regsub)
...
qu2 <- filter(qu_treshold, p_value<0.05) %>% arrange( regsub,threshold)
