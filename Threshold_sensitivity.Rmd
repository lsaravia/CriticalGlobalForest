...
#
pyfit_threshold %>% filter( threshold %in% c(20,25,30,35,40)) %>% group_by(model_name) %>% mutate(tot_by_model=n()) %>% 
	filter(delta_AICc==0) %>% summarise(best_model_prop=n()/max(tot_by_model)) %>% select(model_name,best_model_prop)  
...
# linear mixed model with year as Random factor 
#

require(nlme)

am <-gls(par1 ~ regsub*threshold,data=ff,weights = varFixed(~sd_alpha))
summary(am)
plot(am, which=1)

ctrl <- lmeControl(opt='optim');
mdl1 <- lme(par1~regsub*threshold, random=~1 | year, data=ff,method="REML",weights = varFixed(~sd_alpha))
summary(mdl1)
plot(mdl1)
mdl2 <- lme(par1~regsub*threshold, random=~1+threshold | year, data=ff,method="REML",control = ctrl,weights = varFixed(~sd_alpha))
summary(mdl2)
anova(mdl2)
plot(mdl2)
e2 <- resid(mdl2,type="normalized")
boxplot(e2 ~ regsub,data=ff)
boxplot(e2 ~ threshold,data=ff)
