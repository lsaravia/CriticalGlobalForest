...
def main():
    ...
    df = pd.DataFrame(columns=['model_name',"par1","par2","xmin","n","LL","AICc","sd_alpha","file_name"])
    ...
    if len(filenames) == 0:
        ...
        for f in filenames:
            ...
            #
            patch = np.fromfile(f, dtype='float64') #
            ...
            line = ["Power",par1,par2,fit.power_law.xmin,n,LL,Akaike,fit.power_law.sigma,f]
            df.loc[len(df)]=line
            ...
            Akaike = AICc(LL,2,n) 
            line = ["LogNormPos",par1,par2,fit.power_law.xmin,n,LL,Akaike,0.0,f]
            df.loc[len(df)]=line
            ...
            # Two parameters for lognormal
            Akaike = AICc(LL,2,n) 
            line = ["LogNorm",par1,par2,fit.power_law.xmin,n,LL,Akaike,0.0,f]
            df.loc[len(df)]=line
            ...
            Akaike = AICc(LL,2,n) 
            line = ["PowerExp",par1,par2,fit.power_law.xmin,n,LL,Akaike,0.0,f]
            df.loc[len(df)]=line
            ...
            Akaike = AICc(LL,1,n) 
            line = ["Exp",par1,par2,fit.power_law.xmin,n,LL,Akaike,0.0,f]
            df.loc[len(df)]=line

 
    df.to_csv('fittedDistributions.txt', header=True, index=False, sep='\t', mode='a')    
