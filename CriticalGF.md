...
The second case are continuous critical transitions [@Sole2006], or second order transitions. In this case there is a narrow region where the system changes from one domain to another, besides the change is also sudden it is continuous and theoretically reversible. These transitions are linked to spatial structures: at the critical point we can observe self-similar or scale invariant fractal structures [@Stauffer1994]. When the mechanisms that produce first order transitions are perturbed by demographic fluctuations, spatial heterogeneities or dispersal limitation is present the first order catastrophic transition convert them to second order transitions [@Martin2015]. This kind of fluctuations are ubiquitous in natural systems so the second order transitions should be the most common. 
