...
Therefore, even if critical thresholds are reached only in some forest regions at a continental scale, a cascading effect with global consequences could still be produced, and may contribute to reach a planetary tipping point (LEO TODO - I THOUGHT YOU SAID THIS LANGUAGE WAS REMOVED?) [@Reyer2015]. The risk of such event will be higher if the dynamics of separate continental regions are coupled [@Lenton2013]. At least three of the regions defined here are considered tipping elements of the earth climate system that could be triggered during this century [@Lenton2008]. These were defined as policy relevant tipping elements so that political decisions could determine whether the critical value is reached or not. Thus using the criteria proposed here could be used as a more sensitive system to evaluate the closeness of a tipping point at a continental scale, but the same criteria could also be uses to evaluate local problems at smaller areas. Further improvements will produce quantitative predictions about the temporal horizon where these critical transitions could produce significant changes in the studied systems. 
