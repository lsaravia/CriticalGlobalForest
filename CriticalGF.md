...
To select the best model we calculated corrected Akaike Information Criteria ($AIC_c$) and Akaike weights for each model [@Burnham2002]. Akaike weights ($w_i$) are the weight of evidence in favor of model $i$ being the actual best model given that one of the $N$ models must be the best model for that set of $N$ models. Additionally, we computed a likelihood ratio test [@Clauset2009; @Vuong1989] of the best model obtained by $AIC_c$ against the alternatives. 
