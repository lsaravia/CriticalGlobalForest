...
The largest patch is the one that connects the highest number of sites in the area, it has been used extensively to indicate fragmentation [@Ochoa-Quintero2015; @Gardner2007]. The relation of the size of the largest patch $S_{max}$ with critical transitions has been extensively studied in relation to percolation phenomena [Bazant2000;Botet2004;@Stauffer1994], but seldom used in ecological studies (but see [-@Gastner2009]). When we are in a connected state ($p>p_c$) the landscape is almost insensitive to removal experiments, but close to the critical point the removal of a number of small patches can have an important effects [@Sole2006; @Oborny2007]. Thus the biggest patch will have big fluctuations if we are near the critical point. We consider that the maximum patch is a realization of a stochastic process with a power law distribution given by equation (@e1), then temporal fluctuations of $S_{max}$ near the critical transition should also follow power law [@Botet2004]. 

To calculate the fluctuations we first divided $S_{max}$ by the total forest area for this year and then the differences between years: $\Delta S_{max}=S_{max}(t2)-S_{max}(t1)$. We fitted three empirical distributions: power-law, log-normal, and exponential; using the same methods described previously. As the data set spans 15 year we have only 14 points to fit the distributions, the estimated parameters are not reliable and the power to detect which distribution fits better data is low. To improve this we performed the goodness of fit test for all the distributions. 
...
These critical transitions predict when the fragmentation of the forest increase suddenly in response to a small increase of human perturbation.
