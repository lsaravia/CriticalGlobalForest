...
#
# modType 4: Hierarchical neutral saturated
#         3: Hierarchical non saturated
#
simul_NeutralPlotTime <- function(nsp,side,disp,migr,repl,birth=1,death=0.2,meta="U",modType=4,simul=T,timeEnd=1000,sims=10,mf="N",clus="S",sedIni=0,timeInit=1,timeDelta=10,rndSeed=0) {
  ...
  if(toupper(meta)=="L") {
    ...
    neuParm <- paste0("fishP",nsp,"_",side,"R", repl, "m",migr)
    bname <- paste0("neuFish",nsp,"_",side,"R", repl, "m",migr)
  } else {
    if(simul) prob <- rep(1/nsp,nsp)  
    neuParm <- paste0("unifP",nsp,"_",side,"R", repl,"m",migr)
    bname <- paste0("neuUnif",nsp,"_",side,"R", repl,"m",migr)
  }
  pname <- paste0("pomacR",repl,".lin")
  ...
  if(simul){
    ...
    par[par$V1=="modType",]$V2 <- modType   # 4=Hierarchical saturated
  ...
  if(!simul) {
    ...
    require(tidyr)
    den_l<- gather(den,species,prop,starts_with("X")) %>% group_by(Rep,species) %>% filter(mean(prop)>0.01) #%>% filter(Time>tini) 
    print(ggplot(den_l, aes(x=Time, y=prop,color=species)) + facet_wrap(~Rep, ncol=2 ) + scale_colour_brewer(palette="Dark2")  + guides(color=FALSE) + ylab("Species densities") +
            geom_line() + theme_bw() +  ggtitle(paste("Side:",side,"Ro:",repl,"m:",migr)))

        print(ggplot(den, aes(x=Time, y=Richness,color=factor(Rep))) + scale_colour_brewer(palette="Dark2") +
            geom_line() + theme_bw() +  ggtitle(paste("Side:",side,"Ro:",repl,"m:",migr)) + guides(color=FALSE)) 
    
    print(ggplot(den, aes(x=Time, y=H,color=factor(Rep))) + scale_colour_brewer(palette="Dark2") + ylab("Diversity (H)") +
            geom_line() + theme_bw() +  ggtitle(paste("Side:",side,"Ro:",repl,"m:",migr)) + guides(color=FALSE)) 
...
# Simulate a time series of the neutral/hierarchical model and calculate SAD
#
# disp: dispersal parameter
# migr: migration rate
# repl: replacement rate
...
#         3: Hierarchical non saturated
#
SAD_NeutralPlotTime <- function(nsp,side,disp,migr,repl,birth=1,death=0.2,meta="U",modType=4,simul=T,timeEnd=1000,sims=10,mf="N",clus="S",sedIni=0,timeInit=1,timeDelta=10,rndSeed=0) {
  ...
  if(toupper(meta)=="L") {
    if(simul) prob <- genFisherSAD(nsp,side)
    neuParm <- paste0("fishP",nsp,"_",side,"R", repl, "m",migr)
    bname <- paste0("neuFish",nspSAD_NeutralPlotTime,"_",side,"R", repl, "m",migr)
  } else {
    if(simul) prob <- rep(1/nsp,nsp)  
    neuParm <- paste0("unifP",nsp,"_",side,"R", repl,"m",migr)
    bname <- paste0("neuUnif",nsp,"_",side,"R", repl,"m",migr)
  ...
  require(sads)
  require(tidyr)
  timeEnd <- max(den$Time)
  den_l<- den %>% filter(Time==timeEnd) %>% gather(species,prop,starts_with("X")) %>% group_by(GrowthRate,MortalityRate,DispersalDistance,ColonizationRate,species) %>% summarize(n=n(),mprop=mean(prop)) %>% mutate(mprop= round(mprop*side*side)) 
  
  require(sads)
  
  den_oc <- octav(den_l$mprop)
  den_ra <- rad(den_l$mprop)

  fit_ls <- fitsad(den_l$mprop[den_l$mprop>0],sad="ls")
  fit_bs <- fitsad(den_l$mprop[den_l$mprop>0],sad="bs")
  fit_poilog <- fitsad(den_l$mprop[den_l$mprop>0],sad="poilog")
  #fit_nbinom <- fitsad(den_l$mprop[den_l$mprop>0],sad="nbinom")
  #fit_power <- fitsad(den_l$mprop[den_l$mprop>0],sad="powbend")
  plt_den <-data.frame(radpred(fit_poilog),type="poisson-lognormal",AICc=bbmle::AICc(fit_poilog))
  plt_den <-rbind(plt_den,data.frame(radpred(fit_ls),type="logseries",AICc=bbmle::AICc(fit_ls)))
  plt_den <-rbind(plt_den,data.frame(radpred(fit_bs),type="broken-stick",AICc=bbmle::AICc(fit_bs)))
  #plt_den <-rbind(plt_den,data.frame(radpred(fit_power),type="powbend",AICc=bbmle::AICc(fit_power)))
  plt_den <-arrange(plt_den,AICc)
  plt_den1 <-data.frame(den_ra,type="data")

  
    # Select the last time and make averages by different simulations
  #
  if(!simul) {
    require(ggplot2)
    if(sims>10)  
    ...
    # 
    # plot(den_oc,main=paste("Side:",side,"Ro:",repl,"m:",migr))
    # lines(octavpred(fit_ls ),col=myCols[1],pch=19)
    # lines(octavpred(fit_bs ),col=myCols[2],pch=19)
    # lines(octavpred(fit_poilog ),col=myCols[3],pch=19)
    # lines(octavpred(fit_power ),col=myCols[4],pch=19)
    # legend("topright",
    #        c(paste("Logseries",round(bbmle::AICc(fit_ls))),
    #          paste("Broken-stick",round(bbmle::AICc(fit_bs))),
    #          paste("Poison lognormal",round(bbmle::AICc(fit_poilog))),
    #          paste("Power",round(bbmle::AICc(fit_power)))),
    #        lty=1, col=myCols)
    # 
    # plot(den_ra,col=myCols[5],pch=19)
    # lines(radpred(fit_ls ),col=myCols[1],pch=19)
    # lines(radpred(fit_bs ),col=myCols[2],pch=19)
    # lines(radpred(fit_poilog ),col=myCols[3],pch=19)
    # lines(radpred(fit_power ),col=myCols[4],pch=19)
    # legend("topright",
    #        c(paste("Logseries",round(bbmle::AICc(fit_ls))),
    #          paste("Broken-stick",round(bbmle::AICc(fit_bs))),
    #          paste("Poison lognormal",round(bbmle::AICc(fit_poilog))),
    ...
    print(ggplot(plt_den,aes(rank,abund,colour=type)) + scale_colour_brewer(palette="Dark2")  +
            geom_line(linetype=2) + theme_bw() + scale_y_log10() + geom_point(data=plt_den1) + coord_cartesian(ylim = range(plt_den1$abund))+ ggtitle(paste("Side:",side,"Ro:",repl,"m:",migr)) )   
  }
  plt_den1$AICc<-0
  plt_den<-rbind(plt_den,plt_den1)
  plt_den<-mutate(plt_den, GrowthRate=birth,MortalityRate=death,DispersalDistance=disp,ColonizationRate=migr,ReplacementRate=repl )
}
...
plot_NeutralSmaxDens <- function(nsp,side,disp,migr,repl,meta="L",death=0.2,birth=1,colPal=0,timeStat=50) {
  ...
  g <-ggplot(clu,aes(Dens,Smax_prop)) + geom_point(color=colPal) + theme_bw() + xlab("Densidad") + ylab("Parche máximo") # +  coord_cartesian(xlim=c(0.3,0.8))
...
plot_NeutralSmaxCluster <- function(nsp,side,disp,migr,repl,time=10,inter=1,init=1,meta="L",death=0.2,birth=1,colPal=0) {
  ...
  #    plot(r1, axes=FALSE,box=FALSE,legend=FALSE)
    ...
    g <- ggplot(spa, aes(x, y, fill = factor(v))) + geom_raster(hjust = 0, vjust = 0) + 
      theme_bw() + coord_equal() 
    g <- g + scale_fill_manual(guide=F,values=mc) +#gradientn(colours=mc,guide=F) + #guide="colourbar",name="Species no."
      scale_x_continuous(expand=c(.01,.01),breaks=NULL,limits = c(1,side)) + 
      scale_y_continuous(expand=c(.01,.01),breaks=NULL,limits = c(1,side)) +  
      labs(x=NULL, y=NULL) +theme(panel.border = element_blank()) +
      annotate("text",x=20,y=-15,size=4,label=paste0("Time: ",tra[t])) +
